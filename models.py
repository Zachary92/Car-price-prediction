...
X=pd.concat([car_brand2,car_model2,fuel_type2,cars[['highwaympg','citympg']]],axis=1)
Y=cars['price']
train_X,test_X,train_Y,test_Y=train_test_split(X,Y,random_state=0)

#decision tree model
Input1=[('scaler',StandardScaler()),('model',DecisionTreeRegressor())]
...
rscore_results=pd.DataFrame({'Linear':linear,'Polynomial': polynomial,
                             'DecisionTree':decision_tree,'RandomForest':random_forest},index=[0])
